...
export default function App() {
  ...
  const checkAuthStatus = async () => {
    ...
    try {
      ...
      if (authenticated) {
        ...
        console.log('[SidePanel] Current user:', user);
        if (user) {
          setCurrentUser({ username: user.username, userId: user.userId });
        }
      } else {
        setCurrentUser(null);
        console.log('[SidePanel] No user authenticated');
      }
    } catch (error) {
  ...
  const handleManualLogout = async () => {
    console.log('[SidePanel] Manual logout initiated...');
    setIsCheckingAuth(true);
    try {
      await ExtensionAuthService.logout();
      console.log('[SidePanel] Manual logout completed');
      await checkAuthStatus();
    } catch (error) {
      console.error('[SidePanel] Error during manual logout:', error);
      setIsCheckingAuth(false);
    }
  };

  const handleGoToLogin = () => {
    console.log('[SidePanel] Opening login page...');
